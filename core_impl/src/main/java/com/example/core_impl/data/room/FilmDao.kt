...
interface FilmDao {
    ...
    fun insertFilms(films: List<FilmImpl>)

    @Query("DELETE FROM ${FilmImpl.Fields.TABLE_NAME}")
    fun clear()

    @Query("DELETE FROM ${FilmImpl.Fields.TABLE_NAME} WHERE ${FilmImpl.Fields.IS_FAVORITE} = 0")
    fun clearNotFavorite()

    @Delete
    fun deleteFilm(film: FilmImpl)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertFilm(film: FilmImpl)
}